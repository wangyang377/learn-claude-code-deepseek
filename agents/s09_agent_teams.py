...
class TeammateManager:
    ...
    def _teammate_loop(self, name: str, role: str, prompt: str):
        teammate_client = LLMClient(f"s09-{name}")
        system = [
            {
                "role": "system",
                "content": (
        ...
        for _ in range(50):
            ...
            try:
                response = teammate_client.chat(system + messages, tools)
            except Exception:
                break
            ...
            for tool_call in response.tool_calls:
                output = self._exec(name, tool_call.name, json.loads(tool_call.arguments))
